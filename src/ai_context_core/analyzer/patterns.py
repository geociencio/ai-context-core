    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if inner_func:
                ...
                returns_inner = False
                for item in node.body:
                    if isinstance(item, ast.Return) and isinstance(item.value, ast.Name) and item.value.id == inner_func.name:
                        returns_inner = True
                        break
                
                if returns_inner:
                    evidence.append(f"Function '{node.name}' contains and returns inner function '{inner_func.name}'")
                    confidence += 50
                    
                    # Check for functools.wraps
                    for dec in inner_func.decorator_list:
                        if (isinstance(dec, ast.Call) and isinstance(dec.func, ast.Attribute) and dec.func.attr == "wraps") or \
                           (isinstance(dec, ast.Call) and isinstance(dec.func, ast.Name) and dec.func.id == "wraps"):
...
def detect_strategy(tree: ast.AST) -> List[Dict[str, Any]]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            if any(kw in node.name.lower() for kw in ("strategy", "algorithm", "handler", "engine")):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            ...
            stored_attr = None
            for item in node.body:
                if isinstance(item, ast.FunctionDef) and (item.name == "__init__" or "set_" in item.name):
                    for arg in item.args.args:
                        if any(kw in arg.arg.lower() for kw in ("strategy", "algorithm", "engine", "handler", "mode")):
                            has_injection = True
                            evidence.append(f"Strategy injection detected in '{item.name}' via argument '{arg.arg}'")
                            confidence += 30
                            # Try to find which attribute it's stored in
                            for sub in ast.walk(item):
                                if isinstance(sub, ast.Assign) and any(ast.unparse(t) == f"self.{arg.arg}" or arg.arg in ast.unparse(t) for t in sub.targets):
                                    stored_attr = arg.arg # simplified
                                    break
                            break
                    ...
                    if isinstance(item, ast.FunctionDef) and item.name not in ("__init__", "set_"):
                        for sub in ast.walk(item):
                            if isinstance(sub, ast.Call) and isinstance(sub.func, ast.Attribute):
                                if any(kw in ast.unparse(sub.func).lower() for kw in ("strategy", "algorithm", "engine", "handler")):
                                    has_usage = True
                                    evidence.append(f"Strategy call detected in method '{item.name}': {ast.unparse(sub.func)}()")
                                    confidence += 40
                                    break
